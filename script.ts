let originalCanvas = document.getElementById("original") as HTMLCanvasElement;
let originalContext = originalCanvas.getContext("2d", {
  willReadFrequently: true,
}) as CanvasRenderingContext2D;

let transformedCanvas = document.getElementById(
  "transformed"
...
let transformedBackCanvas = document.getElementById(
  "transformed-back"
) as HTMLCanvasElement;
...
let cutoffSlider = document.getElementById("cutoff") as HTMLInputElement;
let cutoffValue = document.getElementById("cutoff-value") as HTMLInputElement;
...
function processImages() {
  let imgData = originalContext.getImageData(
    0,
    0,
...
function transform(imageData: ImageData, limit: number = 0) {
...
function transformBack(imageData: ImageData) {
  let data = imageData.data;
  let newImageData = new ImageData(imageData.width, imageData.height);
  let dataLeft: any[] = new Array(data.length / 2);
  let dataRight: any[] = new Array(data.length / 2);
  for (let i = 0, gs = 0, clr = 0, pos = 0; i < data.length; i += 4) {
    if (pos < imageData.width / 2) {
      dataLeft[gs] = {
        r: data[i + 0],
